...
void MyDisplay() {
	...
	if ((IsSphere) && (IsSmall) && (IsWire)) {
		...
		glutSolidSphere(0.2, 15, 15); // 꽉 찬 작은 원구
	}
	else if ((IsSphere) && (!IsSmall) && (!IsWire)) {
		glutSolidSphere(0.4, 15, 15); // 꽉 찬 큰 원구
	}
	else if ((!IsSphere) && (IsSmall) && (!IsWire)) {
		glutSolidTorus(0.1, 0.3, 40, 20); // 꽉 찬 작은 원환체
	}
	else if((!IsSphere) && (!IsSmall) && (!IsWire)){
		glutSolidTorus(0.2, 0.5, 40, 20); // 꽉 찬 큰 원환체
	}
...
void MySubMenu(int entryID) {
	if (entryID == 1) {
		...
		IsWire = true; // 와이어로 그리기
	}
	else if (entryID == 4) {
		IsWire = false; // 솔리드로 그리기
	}
